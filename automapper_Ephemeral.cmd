...
if (!matchre("$Guild|$guild","Necro")) then put .play
if (matchre("$Guild|$guild","Cleric")) then {
	put relesae cyclic
	put .osrel
}
if ($ephemera = 1) then {
# Requirements(copy/paste these to your cmd line to add the macro & trigger
	#trigger {^You have bound a total of (\w+) little ephemera.} {#var ephemera_count $1}
	#macro {E, Control} {#if {$ephemera = 1}{#var ephemera 0;#echo *** ephemera off}{#var ephemera 1; #echo *** ephemera on}}
	
    if !def(ephemera_count) then send ephemera list all
	action var eph_rarity $1; var eph_name $2; put #log >ephemera.txt %eph_rarity|%eph_name|$zoneid|$roomid|$charactername when ^You detect the presence of (?:an?|some) (common|uncommon|tremendously powerful)(?: ephemeron: the)? (.+?)(?:(?: ephemeron)?([\.\,\!]))
	action put #var ephemera 0 when ^Your senses are dulled by your recent usage of your crystalline fibril
}

# You detect the presence of a tremendously powerful ephemeron: the amorphous jade-green chimera!
# action var Poke%i $1; var CanC%i $2; math i add 1;var iMAX %i when ^You detect the presence of (?:an?|some) (?:common|uncommon|tremendously powerful ephemeron: the) (.+?)(?: ephemeron)(.) 
...
loop:
     gosub wave
     delay 0.1
	if ($ephemera = 1) then gosub ephemera
     goto loop
wave:
     delay 0.0001
...
#debug 10
	wait 1
		matchre ephemera_capture ^You detect the presence
		match ephemera_capture ^You detect the presence
	send eph sense
	matchwait 9
	return
	
ephemera_capture:
	put #ECHO >Output %eph_rarity %eph_name Found!
	wait 1
	
	if ((matchre("%eph_rarity","\bcommon")) || ((matchre("%eph_rarity","\buncommon")) &&  ($ephemera_count > 9)) || ((matchre("%eph_rarity","powerful")) &&  ($ephemera_count > 24))) then {
		send eph capture %eph_name
		
		put #var ephemera 0
		put #log >ephemera.txt %eph_rarity|%eph_name|$zoneid|$roomid|$charactername|captured
		wait 10
